...
<div class="pagination">
    ...
    <c:if test="${block != 0}">
    <a href="${url}?b=${block-1}&page=${(block-1)*size + 1}">&lt;&lt;</a>
    </c:if>

    <c:forEach var="pageNumber" begin="${startPage}" end="${endPage}"> <!-- totalPages - because Page has getpages method -->

        <c:choose>
            <c:when test="${page.number != pageNumber - 1}">
                <a href="${url}?page=${pageNumber}&b=${block}"><c:out value="${pageNumber}" /></a>
            </c:when>
            <c:otherwise>
    ...
    <a href="?b=${block+1}&page="${(block+1)*size + 1}">&gt;&gt;</a>
    </c:if>
